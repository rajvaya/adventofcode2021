 ...
 */
const segments_to_numbers = Object.entries(numbers_as_segments).reduce(
  (map, [num, segs]) => {
 ...
 * }
 */
const numbers_groups_by_segments_count = Object.entries(
  numbers_as_segments
).reduce((obj, [num, segs]) => {
...
function solveWires({ wires, outputs } = {}) {
  ...
  if (
    wiresOverlap({ bottom_wire: number_to_wire.get(2), top_wire: c_or_f[0] })
  ) {
...
const output_sum = input.reduce(
  (sum, input_line) => sum + solveWires(input_line),
  0
);
